module TestVerifyV3 exposing (run)
...
verify model =
    let
        errors =
            ...
            , if model.nextId /= 3 then
                ...
                ++ (case model.todos of
                        [ first, second ] ->
                            [ checkTodo "first" first 1 "Buy milk" False 0 0
                            , checkTodo "second" second 2 "Write tests" True 0 0
                            ]
    ...
    if List.isEmpty errors then
        Script.log "All V3 migration checks passed!"

    else
        BackendTask.fail
            (FatalError.build
                { title = "V3 migration verification failed"
                , body = String.join "\n" errors
                }
            )


checkTodo : String -> Todo -> Int -> String -> Bool -> Int -> Int -> Maybe String
checkTodo label todo expectedId expectedTitle expectedCompleted expectedCreatedAt expectedPriority =
    let
        checks =
            ...
            , if todo.createdAt /= expectedCreatedAt then
                ...
                Nothing
            , if todo.priority /= expectedPriority then
                Just (label ++ ": expected priority=" ++ String.fromInt expectedPriority ++ ", got " ++ String.fromInt todo.priority)
